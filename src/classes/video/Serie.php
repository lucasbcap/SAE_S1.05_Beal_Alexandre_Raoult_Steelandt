...
class Serie
{
     ...
     */
    public function __construct(string $titre, string $img, string $genre, string $publiqueVise, string $descriptif, string $sortie, string $dateAjout, array $listEpisode = [])
    {
    ...
    public static function creerSerie(int $id): Serie
    {
        ...
        $requete = $bdd->prepare("Select serie.titre,descriptif,annee,date_ajout,episode.id,genre,img,publicVise from serie inner join episode on serie.id = episode.serie_id where serie.id=:id");
        ...
        while ($d = $requete->fetch()) {
            if (!$creation) {
                $serie = new Serie($d['titre'],$d['img'], $d['genre'], $d['publicVise'], $d['descriptif'], "ok", $d['descriptif']);
                $creation = true;
            }
